...
def test_global_guard_blocks_multiple_targets(tmp_path: Path):
    ...
    rc = main(["--write-mode", "binary-write", "--size", "1KiB", "--disk-guard-margin", "100000TB", str(d)])
    assert rc != 0
...
def test_global_guard_disabled_allows(tmp_path: Path):
    ...
    a.write_bytes(b"")
    b.write_bytes(b"")
    rc = main(["--write-mode", "binary-write", "--size", "1KiB", "--disk-guard-margin", "100000TB", "--disable-disk-guard", str(d)])
    assert rc == 0
